...
export async function checkQuota(tenantData: any, type: 'tours' | 'bookings', currentCount?: number): Promise<{ allowed: boolean, maxLimit: number, current: number }> {
  try {
    ...
    const planName = (tenantData?.plan || 'starter').toLowerCase();
    const billingInterval = (tenantData?.billingInterval || 'monthly').toLowerCase();
    
    let currentPlanObj = plansData.find((p: any) => p.slug.toLowerCase() === planName && p.interval === billingInterval);
    
    if (!currentPlanObj) {
      currentPlanObj = plansData.find((p: any) => p.slug.toLowerCase() === planName);
    }

    let maxLimit = type === 'tours' ? 10 : 50; // Starter defaults
    
    if (currentPlanObj && currentPlanObj.features) {
