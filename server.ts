...
export async function createServer() {
  ...
  // API Route: Send Tour Proposal Email
  app.post("/api/send-proposal-email", async (req, res) => {
    try {
      const { to, customerEmail, proposal, companyName, companyEmail, companyPhone, companyWebsite, tenantId } = req.body;
      const targetEmail = (to || customerEmail || proposal?.email || '').trim();

      if (!targetEmail) {
        return res.status(400).json({ error: "Recipient customer email address is required." });
      }

      console.log(`[API /api/send-proposal-email] Sending proposal to ${targetEmail}`);
          ...
          <div style="max-width: 680px; margin: 0 auto; background-color: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
            ...
            <div style="padding: 28px;">
              ...
              <div style="margin-top: 24px; padding: 20px; background-color: #0f172a; color: #ffffff; border-radius: 16px; text-align: center;">
                <div style="font-size: 11px; font-weight: 800; text-transform: uppercase; color: #f97316; tracking-wider;">Total Package Investment</div>
              ...
              <div style="margin-top: 32px; padding-top: 20px; border-t: 1px solid #e2e8f0; text-align: center; color: #64748b; font-size: 12px;">
      ...
      const sendResult = await sendEmailViaProvider(config, {
        to: [targetEmail],
      ...
      return res.json({ success: true, message: `Proposal email successfully sent to ${targetEmail}`, sendResult });
    } catch (error: any) {
